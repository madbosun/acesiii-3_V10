...
csb
 ...
 1012 format('*** END')
      ...
      ISIZE=1
      FULLAB=LABEL//'PTGP'
CSS      CALL GETREC(0, 'JOBARC', FULLAB, length, Ijunk)
CSS      Print*, "The length", length, FULLAB, szjunk
      CALL PUTCREC(20,'JOBARC',FULLAB,8,szJunk)
      FULLAB=LABEL//'ORDR'
CSS      CALL GETREC(0, 'JOBARC', FULLAB, length, Ijunk) 
      CALL PUTREC(20,'JOBARC',FULLAB,ISIZE,IORDER)
      FULLAB=LABEL//'NIRR'
      CALL PUTREC(20,'JOBARC',FULLAB,ISIZE,NCLASS)
C
C WRITE OUT THE NUMBER OF ORBITS
C
      FULLAB=LABEL//'NORB'
      CALL PUTREC(20,'JOBARC',FULLAB,ISIZE,IORBIT)
C
C WRITE OUT SYMMETRY OPERATIONS
C
      FULLAB=LABEL//'SYOP'
      ISIZE=9*IORDER*IINTFP
      CALL PUTREC(20,'JOBARC',FULLAB,ISIZE,SYMOPS)
C
C WRITE OUT PERMUTATION VECTORS
C
      FULLAB=LABEL//'PERM'
      ISIZE=IORDER*NATOMS
      CALL PUTREC(20,'JOBARC',FULLAB,ISIZE,IPTR)
C
C WRITE OUT ORBIT CENSUS VECTOR AND ORBIT POPULATION VECTOR.
C
      FULLAB=LABEL//'MEMB'
      ISIZE=NATOMS
      CALL PUTREC(20,'JOBARC',FULLAB,ISIZE,MEMBER)
      FULLAB=LABEL//'POPV'
c      ISIZE=IORBIT
      CALL PUTREC(20,'JOBARC',FULLAB,ISIZE,ORBPOP)
C
C WRITE OUT CLASS TYPE VECTOR
C
      FULLAB=LABEL//'CLSS'
      ISIZE=IORDER
      CALL PUTREC(20,'JOBARC',FULLAB,ISIZE,CLSTYP)
C
