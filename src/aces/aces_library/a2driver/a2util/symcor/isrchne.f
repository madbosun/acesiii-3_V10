...
C VECTOR (IA) NOT EQUAL TO A PARTICULAR VALUE (TARGET).

c The UNICOS man page for isrchne is appended to this file.
c NOTE: Since Cray computers always use 64-bit integers, the vendor's
c       isrchne accepts both integers and floats in the array argument.
c       This version only accepts integer arrays.
      ...
      if (inc.ge.0) then
         if (inc.eq.1) then
            do i=1,n
               if (ia(i).ne.target) go to 2
            end do
         else
            ndx=1
            do i=1,n
               if (ia(ndx).ne.target) go to 2
               ndx=ndx+inc
            end do
         end if
      else
