...
C
C
cPV         CASEI   = NCENA .GT. 0
cPV         CASEII  = NCENB .GT. 0
cPV         CASEIII = NCENC .GT. 0
          casei=.false.
          caseii=.false.
          caseIII=.true.
c          write(*,*)
cPV         WRITE (*,*) ' CASEI   = ',CASEI
cPV         WRITE (*,*) ' CASEII  = ',CASEII
cPV         WRITE (*,*) ' CASEIII = ',CASEIII
c          write(*,*)
...
C
         REORDER = .TRUE.
             DO 3000 NIJBEG = 1,NIJ,NIJBLK
                ...
                MIJCEN  = MIJ * NCENC
                MGIJCEN = NGQP * MIJCEN
                CALL  OED__SOI_3CEN_PCGTO_BLOCKZY
     +
     +                     ( NBATCH,
     +                       NINT1DX,NINT1DY,NINT1DZ,
     +                       ATOMIC,
...
C                 WRITE (*,*) ' Finished sph quart a '
     ...
     +
     +
                 WRITE (*,*) ' Finished normalized cart a '
             END IF
         END IF
C
C
