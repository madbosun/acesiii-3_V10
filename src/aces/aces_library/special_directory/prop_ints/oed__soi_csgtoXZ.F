...
c         WRITE (*,'(a,2I3)') ' Index A,B = ',INDEXA,INDEXB
c         write(*,*)
         IF (EMPTY) THEN
             NBATCH = 0
             RETURN
...
C
C
c         write(*,*) 'forcing EQUALAB to be false'
c           EQUALAB=.false.
         IF (EQUALAB) THEN
...
c          write(*,*)
             DO 3000 NIJBEG = 1,NIJ,NIJBLK
              ...
              CALL  OED__SOI_3CEN_PCGTO_BLOCKxz
     +
     +                     ( NBATCH,
     +                       NINT1DX,NINT1DY,NINT1DZ,
     +                       ATOMIC,
...
c       call print_pv1(zcore(zpbatch),nbatch)
...
c             write(*,*) 'print In'
c             call print_pv(zcore(in),NXYZT,NCTR)
...
c          write(*,*)
             ...
             IN = OUT
             OUT = TEMP
c             call print_pv(zcore(in),NCTR,NXYZT)
         END IF
C
C
...
C
     ...
     +
     +
CSSS                 WRITE (*,*) ' Finished xyz to ry ab '
             ELSE
                 IUSED = 0
                 ZUSED = 0
             END IF
