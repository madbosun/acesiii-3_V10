      SUBROUTINE  ERD__SET_SOI_DERV_ABCD
     +
...
CSSS         write(*,*) 'making ATOM12, ATOM23,ATOM34 false'
CSSS         ATOM12=.false.
CSSS         ATOM23=.false.
CSSS         ATOM34=.false.
CSSS         write(*,*)'ATOM12=',ATOM12
CSSS         write(*,*) 'ATOM23=',ATOM23
CSSS         write(*,*) 'ATOM34=',ATOM34
         ...
         IF (ATOMIC) THEN
             EMPTY = .TRUE.
cPV 
CSSS         write(*,*) 'Prakash, I wish to do atomic calc'
cPV             RETURN
         END IF
...
C
   ...
   12            CONTINUE
         ...
         END IF
cPV       
CSSS         write(*,*) 'wish to keep EQUAL12 false'
         EQUAL12=.false.
cPV
         EQUAL34 = ATOM34
   ...
   32            CONTINUE
         ...
         END IF
cPV
CSSS      write(*,*) 'wish to keep EQUAL34 false'
       EQUAL34=.false.
cPV
...
cPV
CSSS       write(*,*) 'SWAP12 is not allowed'
       SWAP12=.false.
cPV
...
cPV  
CSSS      write(*,*) 'SWAP34 is not allowed'
       SWAP34=.false.
cPV
...
CSSS        write(*,*) 'I am going to make it ABCD and TR1234 false'
        PRIMTYP='ABCD'
        TR1234= .false.
...
cPV this is tricky part 
cPV changing all below zero to 1

         IF (ATOMA .NE. ATOMB) CENEQS (2,1) = 1
         IF (ATOMA .NE. ATOMC) CENEQS (3,1) = 1
         IF (ATOMA .NE. ATOMD) CENEQS (4,1) = 1
         IF (ATOMB .NE. ATOMC) CENEQS (3,2) = 1
         IF (ATOMB .NE. ATOMD) CENEQS (4,2) = 1
         IF (ATOMC .NE. ATOMD) CENEQS (4,3) = 1
cend PV
...
C
     ...
     +               .OR. (SAMECD .AND. DERCX.NE.DERDX)
      ...
      WRITE (*,'(A,4(1x,I3))') ' DERAX,DERBX,DERCX,DERDX = ',
     +                         DERAX,DERBX,DERCX,DERDX
                 WRITE (*,*) ' erd__set_derv_abcd '
      Write(6,*) "I am here"
cPV 
CSSS       write(*,*) ' this is not a acceptable conctrain'
cPV                 STOP
             END IF
     ...
     +                    + DFLOAT (DERDX) * IVSUM4
     +                    + HALF )
c Prakash I do not like this 
c my take to fix this
           if (DERAX .ne. 0) then
                NDERX=1
            elseif (DERBX .ne. 0) then
               NDERX=1
            else
              NDERX=0
           end if
...
C
     ...
     +               .OR. (SAMECD .AND. DERCY.NE.DERDY)
      ...
      WRITE (*,'(A,4(1x,I3))') ' DERAY,DERBY,DERCY,DERDY = ',
     +                         DERAY,DERBY,DERCY,DERDY
                 WRITE (*,*) ' erd__set_derv_abcd '
CSSS        write(*,*) 'lets do this Prakash'
cPV                 STOP
             END IF
...
C
  ...
  100            CONTINUE
         ...
         END IF
CSSS         write(*,*) 'Forcing EMPTY To be false always'
         EMPTY=.false.
